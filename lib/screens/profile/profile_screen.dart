  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Profile', style: GoogleFonts.poppins(color: Colors.white, fontWeight: FontWeight.w600)),
        actions: [
      ...
      body: SingleChildScrollView(padding: const EdgeInsets.all(20), child: Column(children: [
        ...
        GestureDetector(
          onTap: () async {
            ...
            if (img == null) return;
            try {
              await ProfileService().uploadProfilePhoto(user.id, File(img.path));
              await ref.read(currentUserProvider.notifier).load(user.id);
            } catch (e) {
              if (!context.mounted) return;
          ...
          child: Stack(children: [
            CircleAvatar(radius: 52, backgroundColor: AppTheme.primaryLight,
              backgroundImage: user.photoUrl != null ? NetworkImage(user.photoUrl!) : null,
              child: user.photoUrl == null ? Text(user.name[0].toUpperCase(), style: GoogleFonts.poppins(fontSize: 40, fontWeight: FontWeight.w700, color: AppTheme.primary)) : null),
        ...
        Text(user.name, style: GoogleFonts.poppins(fontSize: 20, fontWeight: FontWeight.w700, color: AppTheme.ink900)),
        Text(user.email, style: GoogleFonts.inter(fontSize: 13, color: AppTheme.ink400)),
